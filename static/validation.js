class VideoAnnotationTool {
    ...
    async loadVideos() {
        try {
            const response = await fetch('/api/validation/detect-videos');
            const data = await response.json();
    ...
    loadVideo(index) {
        ...
        this.annotationImage.src = `/api/validation/video/${video.index}/frame/0`;
    ...
    loadVideoFrame(videoIndex, frameIndex) {
        this.videoImage.src = `/api/validation/video/${videoIndex}/frame/${frameIndex}`;
    }
    ...
    async submitAnnotation() {
        ...
        try {
            const response = await fetch('/api/validation/save-annotation', {
                method: 'POST',
    ...
    async saveAnnotation(annotation){
        try{
            await fetch('/api/validation/save-annotation', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify(annotation)
            });
